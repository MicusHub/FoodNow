...
public class LoginTests extends TestBase {
    ...
    public void preCondition() {
        getLogger().info("*** LOGIN TESTING IN PROGRESS ***");
    }

    @Test
    public void loginPositiveTest() {
        getLogger().info("*** LOGIN POSITIVE TESTING IN PROGRESS ***");
    ...
    public void loginNegativeTestWithAssert(String email, String password) {
        getLogger().info("[LoginTests] Starting negative login test with data: email={}, password={}", email, password);
        ...
        if (!actualUrl.equals(loginUrl)) {  // Проверка, что URL не изменился
            getLogger().error("[LoginTests] Test FAILED: Unexpected URL after login with invalid data: " + actualUrl + ", Screenshot: " + app.getBaseHelper().takeScreenshot());
            Assert.fail("Unexpected URL after login with invalid data: " + actualUrl);
        } else {
            getLogger().info("[LoginTests] Invalid login attempt successfuly prevented.");
        }
    ...
    public void postConditions(Method method, ITestResult result) {
        if (result.isSuccess()) {
            getLogger().info("[LoginTests] Test PASSED: " + method.getName());
        } else {
            getLogger().error("[LoginTests] Test FAILED: " + method.getName() + ", Screenshot: " + app.getBaseHelper().takeScreenshot());
        }
